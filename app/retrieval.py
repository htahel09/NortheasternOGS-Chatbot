...
class Retrieval:
    ...
    def search(self, query, k=1):
        ...
        """
        results = [
            self.dataset.documents[i] for i in indices[0] if i < len(self.dataset.data)  # Ensure index is valid
        ]
        urls = [
            self.dataset.urls[i] for i in indices[0] if i < len(self.dataset.data)  # Ensure index is valid
        ]
